...
class Pig { // Declare program class
    ...
    void program() {
        ...
        playerAmount = welcomePrompt(winPts); // Prompts welcome message

        computer = initiateComputer(); // Check if computer is used
        if(computer) {  // Adds object to player objects if computer is used
            playerAmount++;
        }

        Player[] players = new Player[playerAmount];  // Declares player variable
        players = createPlayers(playerAmount, players, computer); // Creates players

        statusMsg(players); // Prompts player scores
        current = players[random(playerAmount)]; // Randomizes starting player 1-

        while(true){                                                            // Gameplay loop
            if (current.playerType == 'h') {                                    // If player is human
    ...
    boolean initiateComputer(){
        boolean computer;

        while(true) {
            out.println("Do you want to add a computer to the game? (y/n)");
    ...
    Player[] createPlayers(int playerAmount, Player[] players, boolean computer) {
        int playerCount = 0;
        if (computer) {
            for (int i = 0; i < playerAmount-1; i++) {                                                 // Creates the 2 players in a loop
                playerCount++;                                                                         // Adds 1 to player objects
                players[i] = new Player(getPlayerName(playerCount), playerCount, 'h');           // Creates player via constructor
            }
            players[playerAmount-1] = new Player("Computer", playerAmount, 'c');
        } else {
            for (int k = 0; k < playerAmount; k++) {                                                   // Creates the 2 players in a loop
                playerCount++;                                                                         // Adds 1 to player objects
                players[k] = new Player(getPlayerName(playerCount), playerCount, 'h');           // Creates player via constructor
            }
        }
        return players;
    }
    ...
    Player round(Player player, char choice, Player[] players) {                // Executes round and return next player
        ...
        if (choice == 'r') {                                                    // If choice parameter is = 'r' then roll the dice
            ...
            if (result == 1) {                                                  // If result is 1 then execute "lose" sequence
                ...
                return players[((player.playerIndex + 1) % players.length)];                 // Switches to next player
            ...
            return players[((player.playerIndex + 1) % players.length)];                     // Return next player
    ...
    int welcomePrompt(int winPoints) {                                            // Used to welcome user into game
        int playerAmount;
    ...
    class Player {                                                              // Declares class for Player
        ...
        Player(String name, int playerCount, char pType){                                   // Declare constructor for Player
